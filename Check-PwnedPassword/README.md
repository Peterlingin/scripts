Check-PwnedPassword
A secure PowerShell function that checks if passwords have been compromised in known data breaches using the Have I Been Pwned Pwned Passwords API.
Features
...
Accepts password input (securely prompted or via parameter)
Computes SHA-1 hash of the password locally on your machine
Sends only the first 5 characters (prefix) of the hash to the HIBP API
Receives a list of hash suffixes that match the prefix
Searches locally for your full hash in the results
Reports whether the password has been compromised and how many times
...
powershell# Returns an object with detailed information
...
powershell# See detailed processing information
...
Parameters
ParameterTypeDescription-PasswordStringThe password to check. If omitted, you'll be prompted securely-QuietSwitchReturns only a boolean ($true if compromised, $false if safe)-AsObjectSwitchReturns a PSCustomObject with IsCompromised, BreachCount, HashSHA1, and DateChecked-VerboseSwitchShows detailed processing information-DebugSwitchShows debug information including hash values
Output Examples
...
Security & Privacy
This script implements the Pwned Passwords API v3 using k-anonymity:
...
Consider the API's usage policy
